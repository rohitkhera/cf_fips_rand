...
{ 
  ...
  if(cfprng_nist_rand(buf, CFPRNG_MAX_RAND_BYTES)==CFPRNG_ERR) {
    cfopenssl_log_err(__FILE__,__LINE__,"cfprng_nist_rand_fail()");
    retVal=CFPRNG_ERR;
    exit(retVal);
  }

  if(cfprng_fips_tests()==CFPRNG_ERR) {
    cfopenssl_log_err(__FILE__,__LINE__,"Failed simple FIPS statistical tests");    retVal=CFPRNG_ERR;
  }
  else
